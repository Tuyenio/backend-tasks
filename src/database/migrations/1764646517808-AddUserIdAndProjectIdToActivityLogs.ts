...
export class AddUserIdAndProjectIdToActivityLogs1764646517808 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const projectIdColumnExists = await queryRunner.hasColumn(
      'activity_logs',
      'projectId',
    );
    if (!projectIdColumnExists) {
      await queryRunner.query(
        `ALTER TABLE "activity_logs" ADD "projectId" uuid`,
      );
    }
    ...
    if (userIdForeignKey) {
      await queryRunner.query(
        `ALTER TABLE "activity_logs" DROP CONSTRAINT "${userIdForeignKey.name}"`,
      );
    }

    // Alter userId column to NOT NULL
    await queryRunner.query(
      `ALTER TABLE "activity_logs" ALTER COLUMN "userId" SET NOT NULL`,
    );
    ...
    if (!indexExists || indexExists.length === 0) {
      await queryRunner.query(
        `CREATE INDEX "IDX_d9164fd61b6f08f6068e9c542e" ON "activity_logs" ("projectId") `,
      );
    }
    ...
    await queryRunner.query(
      `ALTER TABLE "activity_logs" ADD CONSTRAINT "FK_d9164fd61b6f08f6068e9c542ea" FOREIGN KEY ("projectId") REFERENCES "projects"("id") ON DELETE SET NULL ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "activity_logs" DROP CONSTRAINT "FK_d9164fd61b6f08f6068e9c542ea"`,
    );
    await queryRunner.query(
      `ALTER TABLE "activity_logs" DROP CONSTRAINT "FK_597e6df96098895bf19d4b5ea45"`,
    );
    await queryRunner.query(
      `DROP INDEX "public"."IDX_d9164fd61b6f08f6068e9c542e"`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "activity_logs" DROP COLUMN "projectId"`,
    );
  }
}
